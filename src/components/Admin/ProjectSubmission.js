...
import ListProjectSubmission from "./ListProjectSubmission";
...
function ProjectSubmission({ type }) {
  ...
  useEffect(() => {
    ...
    async function fetchUserCoursesData() {
      const data = await Get(true, `get_all_project_submissions`);
  ...
  async function fetchCourseData(allKeywords) {
    const data = await Get(true, `get_all_project_submissions?keywords=${allKeywords}`);
    console.log(data.data.data.data);
    setUserData(data.data.data.data);
        ...
        <main className="Leaderboard-main">
          <div className="l-wrapper">
            ...
            <button className="c-button c-button--primary">Educator LeaderBoard</button> */}
              <h1 className="c-title">{"Project Submissions"}</h1>
            </div>
                    ...
                    <ul className="c-list" id="list">
                      <li className="c-list__item">
                        <div className="c-list__grid">
                          ...
                          <div className="u-text--left u-text--small u-text--medium">Project Submission</div>
                          <div className="u-text--left u-text--small u-text--medium">Submitted By</div>
                          <div className="u-text--left u-text--small u-text--medium">Submitted At</div>
                        </div>
                      </li>
                      <InfiniteScroll dataLength={userData.length} hasMore={!isLast} next={handleFetchMore}>
                        {userData.length > 0
                          ? userData.map((teacher, index) => (
                                <ListProjectSubmission
                                  name={teacher.project.category.title}
                                  img={teacher.project.category.image_url}
                                  creator={teacher.project.category.creator.name}
                                  creator_slug={teacher.project.category.creator.slug}
                                  user={teacher.user.name}
                                  user_slug={teacher.user.slug}
                                  courses={null}
                                  submission={teacher.title}
                                  submission_link={teacher.url}
                                  slug={teacher.project.category.slug}
