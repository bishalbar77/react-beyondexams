...
function UserCategory({ type }) {
  ...
  useEffect(() => {
    ...
    async function fetchUserCoursesData() {
      const data = await Get(true, `get_category_user`);
  ...
  async function fetchCourseData(allKeywords) {
    const data = await Get(true, `get_category_user?keywords=${allKeywords}`);
    console.log(data.data.data.data);
    setUserData(data.data.data.data);
  ...
  return (
    <div>
      ...
      <Matomo pageTitle={"User Course Enrollments"}>
        ...
        <main className="Leaderboard-main">
          <div className="l-wrapper">
            ...
            <button className="c-button c-button--primary">Educator LeaderBoard</button> */}
              <h1 className="c-title">{"User Course Enrollments"}</h1>
            </div>
                    ...
                    <ul className="c-list" id="list">
                      <li className="c-list__item">
                        <div className="c-list__grid">
                          ...
                          <div className="u-text--left u-text--small u-text--medium">Num Videos</div>
                          <div className="u-text--left u-text--small u-text--medium">Num Projects</div>
                          <div className="u-text--left u-text--small u-text--medium">Progress</div>
                          <div className="u-text--left u-text--small u-text--medium">Rating</div>
                          <div className="u-text--left u-text--small u-text--medium">Certificate</div>
                        </div>
                      </li>
                        ...
                        {userData.length > 0
                          ? userData.map((teacher, index) => (
                                <ListCategoryUser
                                  ...
                                  creator={teacher.user.name}
                                  creator_slug={teacher.user.slug}
                                  ...
                                  index={index}
                                  key={teacher.user_id}
                                  certificate_link={teacher.certificate_url}
                                  progress={teacher.progress}
                                  num_videos={teacher.category.video_count}
                                  projects_count={teacher.category.projects_count}
