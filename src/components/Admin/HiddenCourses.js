...
import ListUnitHidden from "./ListUnitHidden";
...
function HiddenCourses({ type }) {
  ...
  useEffect(() => {
    ...
    async function fetchUserCoursesData() {
      const data = await Get(true, `get_hidden_categories`);
  ...
  async function fetchCourseData(allKeywords) {
    const data = await Get(true, `get_hidden_categories?keywords=${allKeywords}`);
    console.log(data.data.data.data);
    setUserData(data.data.data.data);
  ...
  return (
    <div>
      ...
      <Matomo pageTitle={"Hidden courses"}>
        ...
        <main className="Leaderboard-main">
          <div className="l-wrapper">
            ...
            <button className="c-button c-button--primary">Educator LeaderBoard</button> */}
              <h1 className="c-title">{"Hidden courses"}</h1>
            </div>
                    ...
                    <ul className="c-list" id="list">
                      <li className="c-list__item">
                        <div className="c-list__grid">
                          ...
                          <div className="u-text--left u-text--small u-text--medium" style={{paddingLeft:"10px"}}>Creator Details</div>
                          <div className="u-text--left u-text--small u-text--medium">Created At</div>
                          <div className="u-text--left u-text--small u-text--medium">Actions</div>
                        </div>
                      </li>
                      <InfiniteScroll dataLength={userData.length} hasMore={!isLast} next={handleFetchMore}>
                        {userData.length > 0
                          ? userData.map((teacher, index) => (
                                <ListUnitHidden
                                  name={teacher.title}
                                  img={teacher.image_url}
                                  creator={teacher.creator.name}
                                  creator_slug={teacher.creator.slug}
                                  courses={teacher.creator.num_courses || null}
                                  slug={teacher.slug}
                                  lastActive={teacher.created_at}
                                  educatorDetail={teacher.creator ? teacher.creator : null}
                                  studentDetail={teacher.creator ? teacher.creator : null}
                                  index={index}
                                  key={teacher.user_id}
                                />
                              ))
