...
export const AdvancedDrop = (props) => {
  ...
  const handleClick = (event) => {
    ...
    setMenuPosition({
      top:
        props.inputRef.current.offsetTop +
        props.inputRef.current.offsetHeight +
        21,
  ...
  const getTopics = async () => {
    await axios
      .get(`${baseDomain.route}${baseDomain.subRoute}/${jsonData.api.getAllCourses}`)
      .then((response) => {
          ...
          <div className={styles.scrollContainer}>
            ...
            {preId && data.children[0].level > 2 ? (
              <div className={styles.parentEle}>
                ...
                >
                  {jsonData.dropdown.createCourse}
                </span>
              </div>
            ) : null}
              ...
              if (checkSub(e) || e.level === 1) {
                return (
                  <React.Fragment key={i}>
                    ...
                    >
                      <div>{e.title}</div>
                      <img src={require(`../../../../../../../assets/images/icons/${jsonData.images.arrow}`)} alt="" style={{ width: "8px" }} />
                    </div>
                  </React.Fragment>
                );
              } else
    ...
    <div>
      ...
      >
        <div onClick={() => props.onSmallCLose()} className={styles.smallClose}>
          <div>{jsonData.buttons.close}</div>
          <img src={require(`../../../../../../../assets/images/icons/${jsonData.images.arrow}`)} alt="" />
        </div>
        <div className={styles.content}>
          {smallDropTree(allSmallCategories, null)}
        </div>
      </Dialog>
    </div>
  );
